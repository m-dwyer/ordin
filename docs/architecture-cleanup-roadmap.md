...
### Track 3 — Phase schema additions (parked; revisit when demand surfaces)

> **2026-05-14:** Parked. None of these has a concrete driver yet, and a follow-up review found that not all five belong in the domain at all — see the re-classification below. Revisit individually when there's pain pointing at one of them.
...
**Re-classification (which of these belong in `domain/` vs. runtime config):**

| Step | Cross-provider concept? | Belongs in `domain/Phase`? |
|---|---|---|
| 3.1 `effort` | Yes (Claude `--effort`, OpenAI `reasoning_effort`, Anthropic API extended thinking) | Yes, with per-runtime mapping. Normalize values away from Claude-CLI-specific terminology like `"max"` if needed. |
| 3.2 `output_schema` | Yes (OpenAI `response_format`, Anthropic tool-use, Gemini structured output, runtime-side validation works anywhere) | Yes. |
| 3.3 `mcp` | Yes (MCP is a cross-provider standard; AI SDK has `experimental_createMCPClient`) | Yes — the cleanest of the five. |
| 3.4 `max_budget_usd` | Concept-yes, but Claude CLI has native enforcement and others need runtime-side post-invocation checks | Yes, with the runtime-side enforcement story written down per runtime. |
| 3.5 `permission_mode` | **No.** This is Claude Code's interactive-permission concept (prompts the *user* via Claude Code's own UI). Ordin's broker is the permission model; ordin's worker doesn't surface those prompts. | **No.** If we want the flag pass-through at all, it belongs in `runtimes.claude-cli.*` (already opaque to the domain per CLAUDE.md). Dropped from Track 3. |
