...
> **Step-1 addendum (L3a credential isolation, shipped):** the inner now reaches Langfuse through a parent-side `Broker` (`src/broker/`) wired as srt's `parentProxy`. Inner emits OTLP to `http://otel/...` via `HTTP_PROXY`; srt's allowlist gates first, then forwards approved egress to the broker on a localhost TCP port; the broker maps `otel` → `127.0.0.1:3000` and stamps `Authorization: Basic <pk:sk>` from credentials it holds parent-side. In `srt` mode, the worker env is allowlisted (`HOME`, `PATH`, `TERM`, temp vars, locale vars); broker URLs/auth, telemetry credentials, model gateway tokens, and ambient API keys are not passed into the worker. The base diagram below describes the kernel-sandbox + srt baseline; the broker sits in the outer process between srt's HTTP proxy and the upstream Langfuse / LiteLLM containers. See [`sandboxing-implementation.md`](./sandboxing-implementation.md) for the L3a roadmap and [`sandboxing-findings.md`](./sandboxing-findings.md) for why we chose `parentProxy` over `mitmProxy`.

> **Step-2 addendum (L3a audit + broker hardening, shipped):** broker hostname map is now a discriminated union — `forward` (proxy upstream + auth) or `internal` (handled in-broker). The `audit` internal service receives `POST http://audit/events` from the inner and appends sha256-chained envelopes to `<runStoreDir>/<runId>/audit.jsonl`; the broker also records its own `broker.forward` / `broker.connect` observations into the same chain (so SOCKS-through-CONNECT egress is visible too — see Finding 18). Every request to the broker requires `Proxy-Authorization: Basic <ordin:secret>`; the secret is generated per-run and passed via srt's `parentProxy.http` URL userinfo so srt forwards it on every proxied request. Inner-direct calls (which the kernel already blocks per Finding 19) get 407 if they ever reached us. Verify a chain with `ordin audit verify <runId>`.
