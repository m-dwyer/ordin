...
- **`WorkflowLoader` / `createExecutionPlan`** (`src/infrastructure/workflow-loader.ts`, `src/orchestrator/workflow-plan.ts`) — loaders parse YAML into manifest objects; executable validation lives in `createExecutionPlan()`, which rejects duplicate phase ids, invalid `on_reject` targets, and unsupported topology before returning an engine-neutral plan.
