...
- **`GatePrompter`** (`src/gates/types.ts`) — collects a decision from a human reviewer. CLI ships `OpenTuiGatePrompter` in `src/cli/gate-prompters/opentui.ts`, which delegates to the live `OpenTuiRunController` (footer panel + keypress decision); a non-TTY sink rejects gate prompts with a message pointing at HTTP + `ordin remote decide`. Future web/Slack/HTTP clients ship their own prompters. The harness default `gateForKind` returns `AutoGate` for every kind — safe headless behaviour, and prod callers always supply their own resolver.
